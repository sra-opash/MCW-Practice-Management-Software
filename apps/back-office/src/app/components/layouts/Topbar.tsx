...
import { Search, Plus, Share, MessageSquare, User, Menu, LogOut, Settings } from "lucide-react";
import { Button } from "@mcw/ui";
import { Input } from "@mcw/ui";
import { Sheet, SheetContent, SheetTrigger, DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="font-medium h-8 w-8 p-0 rounded-full">
              <Avatar className="h-8 w-8">
                {userImage ? <AvatarImage src={userImage} alt="User" /> : null}
                <AvatarFallback className="bg-[#2d8467] text-white">AN</AvatarFallback>
              </Avatar>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56">
            <div className="flex items-center justify-start p-2">
              <div className="flex flex-col space-y-1">
                ...
                <p className="text-xs text-muted-foreground">alam@mcnultycw.com</p>
              </div>
            </div>
            <DropdownMenuSeparator />
